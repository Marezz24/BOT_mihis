...
function procesarExcels() {
  ...
  function separarNombreCompleto(nombreCompleto) {
    ...
    if (partes.length < 4) {
      throw new Error('El formato del nombre es incorrecto. Se espera: "ApellidoPaterno ApellidoMaterno Nombre1 Nombre2"');
    }
  ...
  function processExcel(filePath) {
    ...
    try {
      ...
      if (workbook.SheetNames.includes('Resumen')) {
        ...
        excelObj.sheets['Resumen'] = { error: 'La hoja "Resumen" no se encontró.' };
      }
      ...
      if (workbook.SheetNames.includes('EvolucionCategoria')) {
        ...
        excelObj.sheets['EvolucionCategoria'] = data.map(row => ({
          fechaEvo: row[0] ? formatDate(row[0]) : null,  // Columna A
          categoriaRDEvo: row[1] || null,  // Columna B
          nomFuncionarioEvo: row[2] || null  // Columna C
      })).filter(row => row.fechaEvo || row.categoriaRDEvo || row.nomFuncionarioEvo); // Filtramos filas vacías
      } else {
        excelObj.sheets['EvolucionCategoria'] = { error: 'La hoja "EvolucionCategoria" no se encontró.' };
      }
      ...
      if (workbook.SheetNames.includes('ReqVentilatorio')) {
        ...
        excelObj.sheets['ReqVentilatorio'] = { error: 'La hoja "ReqVentilatorio" no se encontró.' };
      }
      ...
      if (workbook.SheetNames.includes('EvolucionEstado')) {
        ...
        excelObj.sheets['EvolucionEstado'] = { error: 'La hoja "EvolucionEstado" no se encontró.' };
      }
      ...
      if (workbook.SheetNames.includes('AltasCanceladas')) {
        ...
        excelObj.sheets['AltasCanceladas'] = { error: 'La hoja "AltasCanceladas" no se encontró.' };
      }
      ...
      if (workbook.SheetNames.includes('Traslados')) {
        ...
        excelObj.sheets['Traslados'] = { error: 'La hoja "Traslados" no se encontró.' };
      }
      ...
      if (workbook.SheetNames.includes('HistObserv')) {
        ...
        excelObj.sheets['HistObserv'] = { error: 'La hoja "HistObserv" no se encontró.' };
      }
      ...
      if (workbook.SheetNames.includes('HistNecesidades')) {
        ...
        excelObj.sheets['HistNecesidades'] = data.map(row => ({
          fechaHistNec: row[0] ? formatDate(row[0]) : null,
          indicacionesHistNec: row[1] || null,
          accionHistNec: row[2] || null
        }));
      } else {
        excelObj.sheets['HistNecesidades'] = { error: 'La hoja "HistNecesidades" no se encontró.' };
      }
      ...
      if (workbook.SheetNames.includes('DatosPaciente')) {
        ...
        excelObj.sheets['DatosPaciente'] = { error: 'La hoja "DatosPaciente" no se encontró.' };
      }
